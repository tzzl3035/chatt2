...
- **测试**: Vitest 4.0.18 (单元测试) + Cypress 15.9.0 (E2E 测试) + Playwright 1.58.2
...
│   ├── Header.vue       # 头部组件
│   ├── HelloWorld.vue    # 示例组件
│   ├── MessageItem.vue  # 消息项组件（支持文件附件、图片预览、气泡样式）
...
- **时间显示**: 显示完整的年月日时分格式（如 "2024年2月10日 14:30"）
- **消息气泡**: 
  - 他人消息：灰色背景 (#e8e8e8)，带阴影效果
  - 自己消息：深色背景，文字反白
  - 短消息宽度自适应（width: fit-content），避免右边留空
- **用户头像**: 显示用户名首字母作为头像
...
项目包含 Supabase Edge Functions 支持，位于 `supabase/functions/` 目录：

- **register**: 用户注册函数，处理用户注册逻辑
...
- **消息时间显示**: 改为完整的年月日时分格式（如 "2024年2月10日 14:30"）
- **消息气泡样式**: 
  - 他人消息使用灰色背景 (#e8e8e8)，增强对比度
  - 消息宽度自适应内容（width: fit-content），避免短消息右边留空
- **音频播放器**: 增加最小宽度至 400px，确保播放器完整显示
- **移除相对时间**: 不再显示 "5分钟前" 等相对时间，直接显示完整时间

### 测试工具集成

- **Playwright**: 已集成 @playwright/test 1.58.2 用于前端测试
- **Cypress**: 保留用于 E2E 测试
- **Vitest**: 用于单元测试